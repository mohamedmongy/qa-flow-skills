...
Only when the user's scenarios describe **many variants of the same request** (e.g. "sign up 50 users from our spreadsheet", "one call per account in this table") add, as its own message: **"Should this case run once per row of a dataset (data-driven)? (1) no · (2) an Assets Manager file (CSV / XLSX / JSON) · (3) a saved database query · (4) inline rows"** — then group source, `on_row_failure`, and any `limit`/`filter`/`types` into one follow-up, run `preview_dataset` once to show the columns, then `infer_dataset_mapping(kind="test_case", name=<api>, test_case=<case>, binding=…)` and confirm the proposed `column → destination.path` rows; they are saved as the binding's `mapping`. The case keeps its **literal** values — a mapped key takes the row's column, every other key sends the case's own value on every row, and ▶ Run (no data) sends them all. **Never write `{{data.<column>}}` into a test case.** See `ai-rules/reference/dataset-binding.md`. Do not offer a dataset for ordinary single-scenario cases. Running an existing case once per row **without** saving a binding ("run login for every row of this CSV") is not an API change — it is a data-driven `run_api`, governed by `ai-rules/safeguard.md` Rule 6.8.
...
- `dataset`: optional — data-driven binding; the case runs once per row (asset file / saved query / inline rows) the binding's `mapping` (`[{"column", "destination": "payload"|"params"|"headers", "path"}]`) says which column replaces which **literal** of the case; unmapped keys keep the case's value. Never write `{{data.<column>}}` into the case. Schema and rules: `ai-rules/reference/dataset-binding.md` (MCP resource `qa-flow://schemas/dataset`); preview with `preview_dataset`
