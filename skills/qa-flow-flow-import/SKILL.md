...
5. **Missing subflows block at their step's turn.** They are never guessed or synthesized — offer: import their own steps doc first / build via standard negotiation / inline the steps. Collisions (same name, different content) always resolve to a NEW name — never overwrite. Matching content is reused (idempotent re-import).
