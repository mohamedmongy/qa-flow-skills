...
- **Env vars** → [ai-rules/negotiation/env-vars.md](ai-rules/negotiation/env-vars.md) — `{{env.*}}` existence, env-var values and secrets, creation order, the restart step.
...
7. **Build in dependency order after the yes**: env vars (`manage_environment_variables`, per `env-vars.md` — restart a dashboard that predates them before the APIs) → saved queries (`save_query`, each smoke-tested with `test_query`; a failure pauses the build at a reported ✅/⏸ checkpoint — no silent rollback, no auto-delete) → APIs (`create_or_update_api`, test-case additions are additive) → **one** `create_flow` → `validate_flow`.
