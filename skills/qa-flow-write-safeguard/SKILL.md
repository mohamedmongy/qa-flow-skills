...
5. **A data-driven `run_api` is one real request (and one prerequisite chain) per row.** `test_case` names ONE case; confirm it with the dataset source and row count (`preview_dataset`), each `column → destination.path` mapping row, and sequential vs parallel (`dataset_max_parallel`). A missing `dataset` with other `dataset_*` args runs the case's saved binding. Nothing is saved on the case — persisting is `create_or_update_api` under the API negotiation. Report `dataset_summary` after.
6. **`clear_jobs` is irreversible** — it erases the run history the report dashboard is built on. State that plainly before proceeding.
7. **`restore_flow_backup` overwrites the current flow** — state which backup (from `list_flow_backups`) will replace it.
8. **Never hand-edit generated files** (`.py`, `.json`, test suites) — the MCP tools own them (Rules 1–3).
9. **Report the real result** — deleted/skipped counts, file paths, job ids — and never auto-chain another action.
