...
5. **The flow** — **one** `create_flow` call with all steps, wiring, inputs, and session config as confirmed.
6. **Validate** — `validate_flow` immediately after creation; report the result.
...
- **Subflows are NOT self-describing.** The document names them (e.g. "subflow `login_v1`") without defining their internals — a missing subflow can never be guessed or synthesized. It is a **blocking gap**, presented at that step's turn with three options: **(a)** the user points at that subflow's own steps document → import it first (depth-first, this same rule); **(b)** the user picks an **existing flow** to serve as the subflow — suggest near-purpose candidates found in the resolution pass (e.g. a flow already used for the same job in another flow); **(c)** the user describes the subflow's steps (e.g. *"get csrf token, then login"*) → those steps become a **new flow of its own**, created under the document's subflow name (anything the description leaves open is negotiated per `flow.md`'s question set for those steps), built before the parent in Step 6 order.
  **Whichever option resolves it, the parent keeps its `flow` step exactly as the document specifies — user-described steps are NEVER flattened into the parent as individual steps.** Inlining into the parent happens only when the user *explicitly* asks to restructure that way, and it is confirmed as a deviation from the document. The resolved subflow must export every context key the parent consumes from that step (e.g. `accessToken`), per *Context Wiring Integrity*. The parent import stays parked at that step until resolved.
