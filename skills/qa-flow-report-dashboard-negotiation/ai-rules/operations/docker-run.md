...
3. **`.env`** — no configuration or credential is baked into the image (the artifacts are, the settings that point them at a backend are not); every URL, credential and channel setting comes from here. Confirm each group with the user: base URLs, database, report channels. **Start with `DB_TYPE=file_storage`** unless the user needs real data checks — the container is then self-contained. A configured-but-unreachable database makes the dashboard exit at startup; that is the framework being strict, not a bug. Variables that artifacts reference — `{{env.*}}` syntax, secrets and the `sensitive` flag, which environment they must exist in — follow `negotiation/env-vars.md`.
...
**What the image itself carries.** Since the artifact-packaging change the published image ships the framework repo's own artifacts — flows, their generated suites and API wrappers, API definitions, test groups, saved queries, run plans. Where that shows up depends on the layout in Rule 1: a **scaffolded project** bind-mounts its own folders over those paths, so the user still sees only their own work (bind mounts never seed); a **standalone folder** on a first `up` gets the image's artifacts copied into its fresh named volumes, and that is the demo content, not the user's.

`docker compose exec dashboard seed_artifacts.sh` copies the image's baked artifacts into the mounted paths — missing files only, `--force` to overwrite, `--dry-run` to list; it never deletes, so artifacts the user created and edits they made are kept. It exists for the case the first-mount rule leaves stranded: a deployment whose volumes predate the image. **It is not part of a normal upgrade**, and in a scaffolded project it would drop the framework's demo artifacts straight into the user's git folder — so propose it only when the user actually wants the image's artifacts, and `--dry-run` first.
...
2. **The checkout's artifacts overlay the image's**, `docker cp`-ed in: `code_generator/`, `user_data/`, `user_queries/`, `Configs/`, plus the plan folder. The image ships the artifacts of the release it was built from, so a plan resolves even with nothing copied in — which is precisely why dropping one of these copies fails *silently*, running the released version of an artifact instead of the one on this commit, rather than raising the import error it used to.
...
- **Always pass `--target`.** `test` is the last stage, so a bare `docker build` produces the test variant, not the consumer image. `runtime` → the framework plus the repo's own artifacts (flows, API definitions, test groups, saved queries, run plans — not assets, saved environments or prerequisite templates, which carry credentials or aren't in git), published as `:latest` / `:X.Y.Z`. `test` → runtime + the framework's own suites, published as `:latest-test` / `:X.Y.Z-test`.
...
- **Verify before publishing**: the framework suites inside the image (`run_framework_tests.sh` on the `-test` tag), a running-container probe on a free host port with `DB_TYPE=file_storage` (`validate_container.sh`, after the container reports `healthy`, whose artifact inventory shows what the image can actually run), and `python scripts/audit_dependencies.py --image …`.
