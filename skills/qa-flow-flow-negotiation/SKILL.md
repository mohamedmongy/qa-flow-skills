...
1. **One pre-negotiation tool call only — the name-availability check.** On a flow/API create request, first detect the intended name (from the request or the cURL), call `list_flows` / `list_api_definitions` once to check it, then send the FIRST response: a single negotiation question proposing the name if it's free, or suggesting 2–3 alternatives if it's taken (a collision always means a new name — never overwrite or update the existing artifact as a fallback). Nothing else runs before negotiation — not `health_check`, not other `list_*`/`get_*`, not `validate_*`. "Gathering context first" beyond that single check is the forbidden pattern itself.
